...
const REDIRECT_URI = TIKTOK_REDIRECT_URI || "https://api.alchemylab.app/tiktok-oauth-callback";
...
// Exchange authorization code for tokens (TikTok Marketing API)
const exchangeCode = async (code: string) => {
  ...
  return {
    ...
    advertiserId: data.data.advertiser_ids?.[0] as string | undefined,
  };
};
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (orgErr || !org) {
      return json({ error: "Could not fetch organization details" }, 500, origin);
    }
    ...
    if (!existingConnection && currentConnections >= planLimit) {
      return json({
        ...
        message: `Your ${org.plan} plan allows ${planLimit} ad account${planLimit !== 1 ? "s" : ""}. Please upgrade.`,
      }, 403, origin);
    }
    ...
    return json({
      success: true,
      message: "TikTok Ads connection established successfully",
